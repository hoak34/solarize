...
class CT_BOLDER_Footer_about extends WP_Widget {

	function CT_BOLDER_Footer_about() {

		$widget_ops = array('classname' => 'CT_BOLDER_Footer_about', 'description' => 'The top footer contact' );

		parent::__construct('CT_BOLDER_Footer_about', CORONATHEMES_THEME_NAME.' footer contact ', $widget_ops);
	...
	function widget($args, $instance) {
		...
		$email = $instance['email'];
			...
			echo '<div class="solarize-company-info widget">
						<h3>'.$title.'</h3>
						<p>'.$description.'</p>					
	...
	function form($instance) {
		...
		?>
			...
			<p><label for="<?php echo $this->get_field_id('title'); ?>">Title (default "About company"): <input class="widefat" id="<?php echo $this->get_field_id('title'); ?>" name="<?php echo $this->get_field_name('title'); ?>" type="text" value="<?php echo esc_attr($title); ?>" /></label></p>

			<p>
				<label for="<?php echo $this->get_field_id('textarea'); ?>"><?php _e('Description:', 'solarize'); ?></label>
...
function solarize_footer_about_widget() {
	register_widget( 'CT_BOLDER_Footer_about' );
}

add_action( 'widgets_init', 'solarize_footer_about_widget' );