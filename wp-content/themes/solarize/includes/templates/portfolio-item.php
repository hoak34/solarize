<?php 
    ...
    $portfolio_hover_color = get_post_meta( $post->ID, "solarize_portfolio_hover_color", true );
    $p_rgba = hex2rgb($portfolio_hover_color);
    $portfolio_hover_style = 'style="background-color:rgba('.$p_rgba[0].', '.$p_rgba[1].', '.$p_rgba[2].', '.$solarize_config['portfolio_hover_opacity'].')"';
    $url = wp_get_attachment_url( get_post_thumbnail_id($post->ID) );
 ?>

<li class="cbp-item <?php echo solarize_isotope_terms() ?>" style="width:<?php echo esc_attr($solarize_config['portfolio-item-width'] )?>px; height: <?php echo esc_attr($solarize_config['portfolio-item-height']) ?>px">
